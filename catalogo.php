...
$query = "
    ...
    FROM 
        barche b
    WHERE b.stato = 'in vendita' -- <-- MODIFICA CHIAVE QUI
    ORDER BY 
        b.id DESC
        ;
";
...
<head>
    ...
    <title>DreamDay - Catalogo Barche in Vendita</title> <!-- Titolo aggiornato per chiarezza -->
...
<section class="hero-bg relative flex flex-col items-center justify-center h-64 sm:h-72 bg-gradient-to-b from-blue-900 to-blue-700 text-white pt-16">
    <h1 class="text-3xl sm:text-5xl font-extrabold">Le Nostre Barche in Vendita</h1> <!-- Titolo aggiornato -->
...
<section class="py-12 px-4 sm:px-6">
    <div id="catalog-grid" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
        <?php if (!empty($boats)): ?>
                ...
                <div class="p-4">
                    ...
                    <div class="mt-2 mb-3">
                        <span class="text-lg font-bold text-blue-800">
                            <?= isset($b['prezzo']) && $b['prezzo'] !== '' ? 
                                '€' . number_format((float)$b['prezzo'], 0, ',', '.') : 
                                '<span class="text-base font-semibold">Prezzo su richiesta</span>' ?>
                        </span>
                    </div>
                    ...
                    <div id="details-<?= $b['id'] ?>" class="boat-details mt-2 bg-blue-50 p-2 rounded text-sm">
                        <div class="flex items-center mb-2">
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium <?= $b['stato'] === 'in vendita' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' ?>">
                                <?php if ($b['stato'] === 'in vendita'): ?>
                                    🏷️ In Vendita
                                <?php else: ?>
                                    🔄 Da Noleggiare
                                <?php endif; ?>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p class="text-center text-gray-500 col-span-full text-lg">Nessuna barca in vendita trovata al momento.</p>
        <?php endif; ?>
    </div>
</section>
